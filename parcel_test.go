...
func TestAddGetDelete(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, parcel, parcelCheck) // assert.Equal(t, parcel.Client, pars.Client)
...
func TestSetStatus(t *testing.T) {
	...
	pars, err := store.Get(id)
	require.NoError(t, err)
	assert.Equal(t, pars.Status, ParcelStatusSent)
}
...
func TestGetByClient(t *testing.T) {
	...
	// add
	for i := 0; i < len(parcels); i++ { // добавьте новую посылку в БД, убедитесь в отсутствии ошибки и наличии идентификатора
